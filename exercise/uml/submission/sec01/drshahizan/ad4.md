...
| 7. Outline the steps involved in appointment scheduling and how the system handles it. | In appointment scheduling, the system follows these steps:
  1. The user interface displays available appointment slots based on predefined schedules and availability.
  2. The hospital staff selects an appropriate slot for the patient and provides the necessary details (patient ID, doctor ID, date, time, etc.).
  3. The user interface sends the appointment details to the backend through a REST API.
  4. The business logic component receives the appointment details and performs validation, checking for conflicting appointments or other constraints.
  5. If the appointment is valid, the business logic instructs the data access logic component to update the appointment schedule in the database.
  6. The data access logic component updates the appointment record, marking the slot as booked and associating it with the patient and the doctor.
